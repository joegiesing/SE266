    ...
    public function insertSchoolsFromFile($fileName) 
    {
        ...
        {   
            ...
            foreach ($csvAsArray as $i=>$row)
            {
                ...
                $school = str_replace("'", "''", htmlspecialchars($row[$keys[0]]));
                $city = str_replace("'", "''", htmlspecialchars($row[$keys[1]]));
                $state = str_replace("'", "''", htmlspecialchars($row[$keys[2]]));
                ...
                if ($schoolCounter++ < self::MAX_INSERT_ROWS != 0) 
                {
                    ...
                    $insertSucessful = $schoolTable->query("INSERT INTO schools (schoolName, schoolCity, schoolState) VALUES ". $schoolToInsert);
                }
            }
